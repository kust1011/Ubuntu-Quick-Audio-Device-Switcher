...
exclude_dev="GP107GL High Definition Audio Controller Digital Stereo (HDMI)"

# get the list of available sinks and their indexes
mapfile -t sinks < <(pacmd list-sinks | awk '/index/ {print $NF}')

# get the list of available sinks and their device.description
mapfile -t sink_desc < <(pacmd list-sinks | awk -F'"' '/device.description/{print $2}')

# find the indexes of sinks exclude specific device
for i in "${!sinks[@]}"; do
    if [[ "${sink_desc[i]}" == "$exclude_dev" ]]; then
        unset "sinks[i]"
    fi
done
